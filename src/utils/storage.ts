 ...
 */
export async function getUploadChunks(uploadId: string, filename: string): Promise<StorageChunk[]> {
  try {
    ...
    if (error) {
      throw new StorageError(`Failed to list upload chunks: ${error.message}`, undefined, uploadId);
    }

    if (!data || data.length === 0) {
      throw new StorageError(`No chunks found for upload ${uploadId}`, undefined, uploadId);
    }
 ...
 */
export async function downloadChunksAsStream(uploadId: string, filename: string): Promise<string> {
  try {
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      if (error) {
        throw new StorageError(`Failed to download chunk ${chunk.path}: ${error.message}`, undefined, uploadId);
      }

      if (!data) {
        throw new StorageError(`No data received for chunk ${chunk.path}`, undefined, uploadId);
      }
 ...
 */
export async function verifyChunksExist(uploadId: string, filename: string, expectedChunks: number): Promise<boolean> {
  try {
    const chunks = await getUploadChunks(uploadId, filename);
